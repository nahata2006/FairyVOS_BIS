...
bool MapPoint::PreSave(set<KeyFrame*>& spKF,set<MapPoint*>& spMP)
{
    mBackupReplacedId = -1;
    ...
    std::map<KeyFrame*, std::tuple<int,int>> obs = GetObservations();
    for(std::map<KeyFrame*,std::tuple<int,int> >::const_iterator it = obs.begin(), end = obs.end(); it != end; ++it)
    {
